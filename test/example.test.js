...
import { render, findById, calcLineItem, calcOrderItem, addProduct, getLocalStorageGames, setInLocalStorage } from '../utils.js';
import { renderTableRow } from '../cart-page/cart-utils.js';
...
test('addProduct should take in a product object and add it to local storage (returning nothing)', (expect) => {
    ...
    const stringyItem = JSON.stringify(expectation);
    
    localStorage.setItem('BOOKS', stringyItem);
    //Arrange
    // Set up your arguments and expectations    //Act 
    // Call the function you're testing and set the result to a const
    addProduct(newBook, 'BOOKS');
